...
class _EditItemState extends State<EditItem> {
  ...
  void _loadItemData() async {
    try {
      final item = await _source.getItem(widget.itemId);
      if (item != null && mounted) {
        _currentItem = item;

        name.text = item.name;
        printingTime.text = item.printingTime.toStringAsFixed(2);
        weight.text = item.weight.toStringAsFixed(2);
        kgCost.text = item.kgCost.toStringAsFixed(2);
        markup.text = item.markup.toStringAsFixed(2);
        printerPower.text = item.printerPower.toStringAsFixed(2);
        kwhCost.text = item.kwhCost.toStringAsFixed(2);

        printerPrice.text = item.printerPrice?.toStringAsFixed(2) ?? '0';
        lifeTime.text = item.lifeTime?.toStringAsFixed(2) ?? '0';
        failPercentage.text = item.failPercentage?.toStringAsFixed(2) ?? '0';

        _calculateValues(updateFinalPrice: true);

        finalPrice.text = item.finalPrice.toDouble().obterReal();
